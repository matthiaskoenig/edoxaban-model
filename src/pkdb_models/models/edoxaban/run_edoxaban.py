"""Tool to run the edoxaban model factory and simulation experiments."""
...
from pkdb_models.models.edoxaban import EDOXABAN_PATH
from pkdb_models.models.edoxaban.simulations import run_simulation_experiments, EXPERIMENTS
...
def _setup_custom_results_paths(results_dir: str):
    ...
    import pkdb_models.models.edoxaban as edoxaban
    custom_path = Path(results_dir).resolve()
    custom_path.mkdir(parents=True, exist_ok=True)
    # Override the module paths
    edoxaban.RESULTS_PATH = custom_path
    edoxaban.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.edoxaban import RESULTS_PATH
    return RESULTS_PATH
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_edoxaban --action simulate --experiments "misc,LaCreta2016"[/dim]')
    console.print('[dim]Or use "all" to run all experiments: run_edoxaban --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    console.rule("[bold cyan]PBPK/PD MODEL[/bold cyan]", style="cyan")
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {edoxaban.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_edoxaban'.
    ...
    2. Run the Model Factory:
       ...
       $ run_edoxaban --action factory

    3. Run Simulations:
       List available experiments:
       $ run_edoxaban --action list_experiments

       Run all experiments:
       $ run_edoxaban --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_edoxaban --action all
       
       With custom results directory for figures:
       $ run_edoxaban --action all --results-dir '/path/to/my/results'
    """
    main()