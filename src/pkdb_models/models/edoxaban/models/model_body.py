...
_m = Model(
    "edoxaban_body",
    name="Edoxaban body model",
    notes="""
    # Physiologically based pharmacokinetic/ pharmacodynamic (PBPK/PD) model edoxaban (body model)

    Edoxaban, a direct oral anticoagulant (DOAC), is a widely used drug for the prevention and treatment of thromboembolic disorders.
...
SUBSTANCES_BODY = {
    ...
    "mx": {
        ...
        "cinit": 0.0,  # [mmole/l]
        # Molecular weight
        "Mr": 521.0,  # [g/mole]  # FIXME: not known
    },
}
...
kidney_id = "edoxaban_kidney"
liver_id = "edoxaban_liver"
gut_id = "edoxaban_intestine"
...
for skey in ["m4", "m6", "mx"]:
    sdict = SUBSTANCES_BODY[skey]
    _m.species.extend(
        [
            Species(
                f"Cgu_{skey}",
                name=f"{sdict["name"]} (intestinal volume)",
                metaId=f"meta_Cgu_{skey}",
                initialConcentration=0,
                compartment="Vgu",
...
_m.species.extend(
    [
        Species(
            "Cve_edo_total",
            metaId="meta_Cve_edo_total",
            initialConcentration=0.0,
            compartment="Vve",  # Venous plasma
            substanceUnit=U.mmole,
            name="EDO + M4 + M6 + MX (plasma)",
            hasOnlySubstanceUnits=False,
            sboTerm=SBO.SIMPLE_CHEMICAL,
            annotations=[],
        ),
        Species(
            f"Afeces_edo_total",
            metaId=f"meta_Afeces_edo_total",
            initialConcentration=0,
            compartment="Vfeces",
            substanceUnit=U.mmole,
            name=f"EDO + M4 + M6 + MX (feces)",
            hasOnlySubstanceUnits=True,
            sboTerm=SBO.SIMPLE_CHEMICAL,
            annotations=[],
        ),
        Species(
            f"Aurine_edo_total",
            metaId=f"meta_Aurine_edo_total",
            initialConcentration=0,
            compartment="Vurine",
            substanceUnit=U.mmole,
            name=f"EDO + M4 + M6 + MX (urine)",
            hasOnlySubstanceUnits=True,
            sboTerm=SBO.SIMPLE_CHEMICAL,
            annotations=[],
        )
...
_m.rules.extend([
    AssignmentRule(
        "Cve_edo_total",
        "Cve_edo + Cve_m4 + Cve_m6 + Cve_mx",
        unit=U.mmole,
        name="Sum of EDO, M4, M6, MX plasma",
    ),
    AssignmentRule(
        "Afeces_edo_total",
        "Afeces_edo + Afeces_m4 + Afeces_m6 + Afeces_mx",
        unit=U.mmole,
        name="Sum of EDO, M4, M6, MX feces",
    ),
    AssignmentRule(
        "Aurine_edo_total",
        "Aurine_edo + Aurine_m4 + Aurine_m6 + Aurine_mx",
        unit=U.mmole,
        name="Sum of EDO, M4, M6, MX urine",
    ),
])
...
if __name__ == "__main__":
    ...
    from pkdb_models.models.edoxaban.models.model_coagulation import model_coagulation

    model_coagulation.species = [s for s in model_coagulation.species if s.sid != "Cve_edo"]

    result = create_model(
        filepath=MODEL_BASE_PATH / f"{model_body.sid}.xml",
        model=[model_coagulation, model_body], sbml_level=3, sbml_version=2
    )
